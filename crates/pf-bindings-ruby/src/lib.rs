...
use pf_bindings_core::{verify_bet, register_gdp_package, BindingError};

/// Result codes C-compatible enum for Ruby compatibility
#[repr(C)]
#[derive(Debug)]
pub enum PfResult {
    Success = 0,
...
/// Error context structure for Ruby compatibility
#[repr(C)]
pub struct PfError {
...
pub extern "C" fn pf_register_gdp_package(
    ...
    if bytes.is_null() && len > 0 {
        if !error_out.is_null() {
            unsafe {
                (*error_out) = PfError::from_string("null bytes pointer with non-zero length".to_string());
            }
        }
        return PfResult::InvalidInput;
    }
